...
public class TravellerService implements ITravellerService {
    ...
    @Override
    public Optional<TravellerDto> getTraveller(final long id) {
        return travellerRepository.findById(id).map(TravellerMapper::toDto);
    }

    @Override
    public Optional<TravellerDto> getTravellerByEmail(final String email) {
        return travellerRepository.findByEmail(email).map(TravellerMapper::toDto);
    }

    @Override
    public Optional<TravellerDto> getTravellerByMobile(final String mobileNumber) {
        return travellerRepository.findByMobileNumber(mobileNumber).map(TravellerMapper::toDto);
    }

    @Override
    public Optional<TravellerDto> getTravellerByDocument(final DocumentType documentType, final String number, final String country) {
        return travellerRepository.findByDocument(DocumentId.of(documentType, number, country))
